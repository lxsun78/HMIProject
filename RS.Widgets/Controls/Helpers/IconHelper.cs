    ...
    public static class IconHelper
    {
        ...
        private static void EnsureSystemMetrics()
        {
            if (s_systemBitDepth == 0)
            {
                ...
                try
                {
                    int sysBitDepth = NativeMethods.GetDeviceCaps(hdcDesktop, NativeMethods.BITSPIXEL);
                    sysBitDepth *= NativeMethods.GetDeviceCaps(hdcDesktop, NativeMethods.PLANES);
                    ...
                    int cxSmallIcon = NativeMethods.GetSystemMetrics(SM.CXSMICON);
                    int cySmallIcon = NativeMethods.GetSystemMetrics(SM.CYSMICON);
                    int cxIcon = NativeMethods.GetSystemMetrics(SM.CXICON);
                    int cyIcon = NativeMethods.GetSystemMetrics(SM.CYICON);
        ...
        public static void GetDefaultIconHandles(out IconHandle largeIconHandle, out IconHandle smallIconHandle)
        {
            ...
            string iconModuleFile = NativeMethods.GetModuleFileName(new HandleRef());

            // We don't really care about the return value.  Handles will be invalid on error.
            int extractedCount = NativeMethods.ExtractIconEx(iconModuleFile, 0, out largeIconHandle, out smallIconHandle, 1);
        }
        ...
        private static IconHandle CreateIconHandleFromBitmapFrame(BitmapFrame sourceBitmapFrame)
        {
            ...
            {
                throw new Exception("sourceBitmapFrame cannot be null here");
            }
            ...
            bool isIcon)
        {
            ...
            try
            {
                ...
                if (maskArray == null)
                {
                    throw new ArgumentNullException(nameof(maskArray));
                }

                maskBitmap = NativeMethods.CreateBitmap(width, height, 1, 1, maskArray);
                ...
                return NativeMethods.CreateIconIndirect(iconInfo);
            }
            finally
            {
                if (colorBitmap != null)
        ...
        public static BitmapFrame GetBestMatch(ReadOnlyCollection<BitmapFrame> frames, Size size)
        {
            if (size.Width == 0)
            {
                throw new Exception("input param width should not be zero");
            }

            if (size.Height == 0)
            {
                throw new Exception("input param height should not be zero");
            }
