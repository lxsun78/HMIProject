...
namespace RS.WPFClient.Controls
{
    public class RSSerialPort : UserControl
    {
        ...
        public static readonly DependencyProperty ModbusDataConfigModelListProperty =
            DependencyProperty.Register("ModbusDataConfigModelList", typeof(ObservableCollection<ModbusDataConfig>), typeof(RSSerialPort), new PropertyMetadata(null));
        ...
        public static readonly DependencyProperty ModbusDataConfigModelSelectedProperty =
            DependencyProperty.Register("ModbusDataConfigModelSelected", typeof(ModbusDataConfig), typeof(RSSerialPort), new PropertyMetadata(null));
        ...
        private async void DeleteModbusDataConfigModel(string parameter)
        {
            ...
            var operateResult = await this.PART_RSDialog.Loading.InvokeAsync(async (cancellationToken) =>
            {
                ...
                if (parameter.Equals("0"))
                {
                    ...
                    {
                        var count = await db.ModbusCommuConfig.Where(t => t.Id == modbusDataConfigModelSelected.Id).ExecuteDeleteAsync();
